...
func refresh_data(id = character_selected):
	...
	var initialEquipData = character_data["initial_equip"]
	...
	for equip in equip_slots:
		...
		match(kind):
			"w":
				var weapon_id = -1
				if (kind_id < initialEquipData.size()):
					weapon_id = initialEquipData[String(kind_id)]
			...
			"a":
				var armor_id = -1
				if (kind_id < initialEquipData.size()):
					armor_id = initialEquipData[String(kind_id)]
	...
	for i in json_data.size():
		var classData = json_data["class" + String(i)]
		if (i > $ClassLabel/ClassText.get_item_count()):
			$ClassLabel/ClassText.add_item(String(classData["name"]))
		else:
			$ClassLabel/ClassText.set_item_text(i, String(classData["name"]))
			
	if character_data.has("effects"):
		clear_effect_list()
		var effectList = character_data["effects"]
...
func save_character_data():
	var jsonDictionary = get_parent().get_parent().call("read_data", "Character")
	var charaData = jsonDictionary["chara" + String(character_selected)]
	var equipTypeData = charaData["equip_types"]
	var initialEquipData = charaData["initial_equip"]

	charaData["faceImage"] = face_path;
	charaData["charaImage"] = character_path;
	charaData["name"] = $NameLabel/NameText.text
	$CharacterButton.set_item_text(character_selected, $NameLabel/NameText.text);
	charaData["class"] = $ClassLabel/ClassText.selected
	charaData["description"] = $DescLabel/DescText.text
	charaData["initialLevel"] = $InitLevelLabel/InitLevelText.value
	charaData["maxLevel"] = $MaxLevelLabel/MaxLevelText.value
	...
	for i in equip_items:
		...
		match (kind):
			"W":
				equipTypeData["w" + String(i)] = equip_id_array[i]
			"A":
				equipTypeData["a" + String(i)] = equip_id_array[i]
	...
	for i in slot_items:
		initialEquipData[String(i)] = int(initial_equip_id_array[i])
	charaData["initial_equip"] = initialEquipData;
	get_parent().get_parent().call("store_data", "Character", jsonDictionary);
...
func _on_AddButton_pressed():
	...
	jsonDictionary["chara" + String(id)] = characterData
	get_parent().get_parent().call("store_data", "Character", jsonDictionary)
	character_selected = id
	refresh_data()
...
func _on_RemoveCharacterButton_pressed():
	...
	if (jsonDictionary.size() > 1):
		...
		while (chara < jsonDictionary.size() - 1):
			if (jsonDictionary.has("chara" + String(chara+1))):
				jsonDictionary["chara" + String(chara)] = jsonDictionary["chara" + String(chara+1)]
			chara += 1
		jsonDictionary.erase("chara"+String(chara))
		get_parent().get_parent().call("store_data", "Character", jsonDictionary)
...
func _on_EquipList_item_activated(index):
	...
	jsonDictionary = get_parent().get_parent().call("read_data", "System")
	var slotsData = jsonDictionary["slots"]

	var equipTypesData = charaData["equip_types"]
...
func _on_AddEquipTypeButton_pressed():
	...
	var jsonDictionary = get_parent().get_parent().call("read_data", "System")
	var wtypeData = jsonDictionary["weapons"]
		...
		0:
			...
			for i in wTypeData.size():
				if (i > $EquipLabel/AddEquip/EquipLabel/EquipButton.get_item_count() - 1):
					...
					$EquipLabel/AddEquip/EquipLabel/EquipButton.set_item_text(i, String(wTypeData[String(i)]))
		1:
			var aTypeData = jsonDictionary["armors"]
